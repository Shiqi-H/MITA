...
## Configure the LLM Environment (Recommended)

If you need to connect to and call the backend LLM, the project root directory must contain a `.env` file with the following format:
...
# "llm" — local rules handle clear intents first; unclear intents use the backend LLM parser.
#         Spoken info, scene, comparison, and disambiguation responses use backend LLM first,
#         then fall back to local templates if the backend call fails.
# "local" — local keyword/rule parsing only; skip backend LLM response generation and use local templates.
VITE_INTENT_ENGINE=
```

The `.env` file required to run this project can be downloaded from this link:

[Download .env file](https://polimi365-my.sharepoint.com/:u:/g/personal/11154362_polimi_it/IQAVrYLa-ew1RanHhA0KbhLKAfRzFDODf1YSdYMJqXxLLnA?e=JEQozX)
...
- The system supports queries about known plant properties, including drought tolerance, height, lifespan, and medicinal value.
- Property queries use the plant information card and the spoken answer is generated by the backend LLM when available.
...
- When `VITE_INTENT_ENGINE=llm`, information generation first attempts to use the backend LLM interface and falls back to local templates if it fails. In `local` mode, the system skips backend LLM generation and uses local templates directly.
...
- [x] Multi-turn follow-up dialogue: Users can first select a plant and then ask questions such as “What is its medicinal value?”, “What is its height?”, or “How does it compare with the previous one?”. The system automatically maintains the context.
